...
// Legacy SAPL implementation removed - using modular implementation in server.auto-pr.mjs
// Endpoints now handled at lines 3552-3645 with full server.auto-pr.mjs integration
...
// ContextLog cleanup endpoint
app.post('/api/ctx/cleanup', async (_req, res) => {
  try {
...
// ContextLog stats endpoint
app.get('/api/ctx/stats', async (_req, res) => {
  try {
...
setInterval(() => {
  try {
    const result = cleanupOldLogs();
    if (result.deleted > 0) {
      console.log(`ContextLog cleanup: deleted ${result.deleted} files, freed ${Math.round(result.bytes_freed / 1024 / 1024)} MB`);
    }
  } catch (e) {
    console.error('ContextLog cleanup error:', e.message);
  }
...
setTimeout(() => {
  try {
    const result = cleanupOldLogs();
    if (result.deleted > 0) {
      console.log(`Initial ContextLog cleanup: deleted ${result.deleted} files, freed ${Math.round(result.bytes_freed / 1024 / 1024)} MB`);
    }
  } catch (e) {
    console.error('Initial ContextLog cleanup error:', e.message);
  }
