...
// Auto‑PR preview (scaffold; dry‑run only)
app.post('/api/auto_pr/preview', async (req, res) => {
  try {
    ...
    res.json({ ok: true, preview: { title: String(title||'Update docs'), body: String(body||'Generated by TGT'), files: allowed }, blocked });
  } catch (e) {
...
app.post('/api/auto_pr/preview', async (req, res) => {
  try {
    ...
    const allow = (process.env.AUTO_PR_ALLOW || 'README.md,docs/,forgekeeper/.env.example,frontend/test/').split(',').map(s=>s.trim()).filter(Boolean);
    const { title, body, files, edits } = req.body || {};
    const requested = Array.isArray(files) ? files.map(String) : [];
    const allowed = requested.filter(p => allow.some(rule => p === rule || p.startsWith(rule)));
    const blocked = requested.filter(p => !allowed.includes(p));
    ...
    res.json({ ok: true, enabled, dryrun, preview: { title: String(title||'Update docs'), body: String(body||'Generated by TGT'), files: allowed, appendPreviews }, blocked });
  } catch (e) {
    res.status(500).json({ ok: false, error: 'server_error', message: e?.message || String(e) });
  }
});
...
app.post('/api/auto_pr/create', async (req, res) => {
  try {
    ...
    try {
      ...
      stdout += run('git push -u origin HEAD');
    ...
    const labels = (process.env.AUTO_PR_LABELS || 'docs').split(',').map(s=>s.trim()).filter(Boolean);
    const createCmd = `gh pr create --base main --title ${JSON.stringify(String(title||'Update docs'))} --body ${JSON.stringify(String(body||'Generated by TGT'))} ${labels.length?('--label '+labels.map(l=>JSON.stringify(l)).join(' ')) : ''}`;
