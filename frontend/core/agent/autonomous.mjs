...
export class AutonomousAgent {
  ...
  constructor(config) {
    ...
    // Self-evaluator for meta-cognitive monitoring
    this.selfEvaluator = new SelfEvaluator({
      accuracyWindow: 10,
      confidenceCalibration: true,
      enableMetaReflection: true,
    });

    // LLM client module - handles all LLM interactions and prompt building
    ...
    // Memory manager module - coordinates all memory systems
    ...
    // Tool handler module - handles tool planning, inference, and execution
    this.toolHandler = new ToolHandler({
      taskPlanner: this.taskPlanner,
    });

    // Reflector module - handles self-evaluation and meta-cognition
  ...
  async run(task, executor, context) {
    ...
    // Assign loaded memories to instance variables
  ...
  async reflect(context, executor) {
    // [Refactored] Build guidance object for LLMClient
    const guidance = {
    ...
    // [Refactored] Delegate to LLMClient module
   ...
   */
  shouldStop() {
    // [Refactored] Delegate to Reflector module
  ...
  async complete(reason, context) {
    ...
    // [Refactored] Record session using MemoryManager
    const taskType = this.detectTaskType(this.state.task);
    const toolsUsed = [...new Set(this.state.history.flatMap(h => h.tools_used || []))];

    // Extract recovery attempts and error patterns from recent failures
    ...
    // Delegate to MemoryManager for recording
    await this.memoryManager.recordSession(taskType, {
      task: this.state.task,
      task_type: taskType,
  ...
  async saveCheckpoint(checkpointId = null) {
    // [Refactored] Delegate to MemoryManager
    const config = {
      maxIterations: this.maxIterations,
