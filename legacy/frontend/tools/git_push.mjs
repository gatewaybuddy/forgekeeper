...
export const def = {
  ...
  function: {
    name: 'git_push',
    description: 'Push commits to remote repository (requires git credentials configured)',
    parameters: {
      type: 'object',
      properties: {
        repo_path: { type: 'string', description: 'Path to git repository (default: current directory)' },
        remote: { type: 'string', description: 'Remote name (default: origin)' },
        branch: { type: 'string', description: 'Branch name (optional, pushes current branch if not specified)' },
      },
      required: [],
...
export async function run({ repo_path, remote = 'origin', branch } = {}) {
  ...
  try {
    ...
    throw new Error(`git_push error: ${err?.message || err}`);
  }
}
