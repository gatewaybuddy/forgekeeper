...
def execute_pipeline(task: str, state: dict, state_path: Path = STATE_PATH) -> bool:
...
def _execute_pipeline(task: str, state: dict, state_path: Path = STATE_PATH) -> bool:
    """Thin wrapper around :func:`execute_pipeline` for easy patching in tests."""
    return execute_pipeline(task, state, state_path)
